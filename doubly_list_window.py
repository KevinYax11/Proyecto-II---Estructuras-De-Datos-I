...
from PyQt6.QtWidgets import QGridLayout, QMainWindow, QPushButton, QHBoxLayout, QVBoxLayout, QWidget, QToolBar, \
    QLineEdit, QLabel, QFileDialog, QScrollArea
...
class DoublyListWindow(QMainWindow):
    def __init__(self, communicate: Communicate):
        ...
        self.insert_pos_btn = QPushButton('Insertar por posicion')
        self.insert_pos_btn.clicked.connect(self.insert_pos)
        self.delete_pos_btn = QPushButton('Eliminar por posicion')
        self.delete_pos_btn.clicked.connect(self.delete_pos)
        ...
        self.label = QLabel()
        self.label2 = QLabel()
        self.label3 = QLabel('Inserte el dato:')
        self.label4 = QLabel('Inserte la posicion:')

        # variables para la barra de herramientas
        toolbar = QToolBar('doubly_list toolbar')
        ...
        self.btn_layout.addWidget(self.label3, 0, 0)
        self.btn_layout.addWidget(self.text_box, 0, 1)
        self.btn_layout.addWidget(self.insert_head_btn, 0, 2)
        self.btn_layout.addWidget(self.insert_tail_btn, 0, 3)
        self.btn_layout.addWidget(self.delete_head_btn, 1, 0)
        self.btn_layout.addWidget(self.delete_tail_btn, 1, 1)
        self.btn_layout.addWidget(self.label4, 2, 0)
        self.btn_layout.addWidget(self.text_box2, 2, 1)
        self.btn_layout.addWidget(self.insert_pos_btn, 2, 2)
        self.btn_layout.addWidget(self.delete_pos_btn, 2, 3)
    ...
    def insert_pos(self):
        self.list.insert_at_prev(self.text_box.text(), int(self.text_box2.text()))
        self.transversal()
    ...
    def delete_pos(self):
        delete = self.list.remove_at(int(self.text_box2.text()))
        self.label.setText(delete)
        self.response_layout.addWidget(self.label)
        if self.max != 0:
            self.max -= 1
    ...
    def transversal(self, pos=-1):
        ...
        while self.max != self.list.size:
            if self.max != 0:
                ...
                self.dot.edge(f'node{self.max}', f'node{self.max-1}', constraint='false')
            self.max += 1
    ...
    def load(self):
        ...
        if dialog_succesful:
            ...
            with open(file_location, 'rb') as file:
                text1 = str(file.read())
                doubly_list = text1.split(sep="'")[1]
                file.close()
                if doubly_list.split(sep='=')[1] == 'doubly_list':
                    size = doubly_list.split(sep='=')[0]
                    for i in reversed(range(int(size))):
                        if i != 0 and i != 1:
                            self.list.prepend(doubly_list.split(sep='=')[i])
                    self.transversal()
                    self.label.setText('Lista cargada con exito')
                    self.main_layout.addWidget(self.label)
                else:
                    self.label.setText('La estructura seleccionada no es una lista doble')
                    self.main_layout.addWidget(self.label)

    def save(self):
        self.new_win = SaveWindow(self.list.size + 2, 'doubly_list', self.list.transversal())
        self.new_win.show()
