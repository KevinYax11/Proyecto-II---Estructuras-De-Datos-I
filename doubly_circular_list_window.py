...
from doubly_linked_circular_list import DoublyLinkedCircularList
...
class DoublyCircularListWindow(QMainWindow):
    def __init__(self, communicate: Communicate):
        ...
        self.setWindowTitle('Lista circular doblemente enlazada')
        ...
        toolbar = QToolBar('doubly_circular_list toolbar')
    ...
    def transversal(self, pos=-1):
        ...
        if f'node{self.max + 1}' == f'node{self.list.size}':
            self.dot.edge(f'node{0}', f'node{self.max}', constraint='false')
            self.dot.edge(f'node{self.max}', f'node{0}', constraint='false')
        else:
            if self.list.size != 0:
                self.dot.edge(f'node{0}', f'node{self.max-1}', constraint='false')
    ...
    def load(self):
        ...
        if dialog_succesful:
            ...
            with open(file_location, 'rb') as file:
                ...
                file.close()
                if circular_list.split(sep='=')[1] == 'doubly_circular_list':
                    ...
                    self.label.setText('La estructura seleccionada no es una lista circular doble')
                    self.main_layout.addWidget(self.label)

    def save(self):
        self.new_win = SaveWindow(self.list.size + 2, 'doubly_circular_list', self.list.transversal())
        self.new_win.show()
